...
This updated version makes compute_sentiment_score robust to the following issues:
 - Accepts SentimentalAnaliser functions that either return a single float or a tuple
   like (avg_score, scores_list).
 - Passes the GitHub token through to SentimentalAnaliser calls.
 - Returns richer `details` for the sentiment area (per-repo dict with score, n, and
   optional error message).
 - Handles authentication/runtime errors gracefully and keeps the rest of the scoring
   pipeline working.

Usage: replace your existing ScoringSys.py content with this file. The rest of the
project (dashboard and SentimentalAnaliser) should work unchanged; however, for
best results make SentimentalAnaliser.get_user_activity_sentiment(full_name, num_events, token)
return (avg, scores_list) as described in my previous message. This ScoringSys will
also handle the older behavior where only a float is returned.
"""
import os
import math
import json
...
def compute_sentiment_score(username: str, token: str, repo_full_names: Optional[List[str]] = None,
    ...
    Details format per repo:
      { 'score': float, 'n': int, 'error': '...' (optional) }

    This function is robust to SentimentalAnaliser.get_user_activity_sentiment returning
    either a float or a tuple/list (avg, scores_list). It also forwards `token` to the
    SentimentalAnaliser call.
    """
    try:
        # try to initialize nltk in the sentiment module (non-fatal)
        f2.setup_nltk()
    except Exception:
        logger.debug("setup_nltk failed but continuing")
    ...
    if not repo_full_names:
        try:
            ...
            logger.warning(f"Could not retrieve repos from StatusAnaliser: {e}")
            repo_full_names = []

    for full in (repo_full_names or []):
        try:
            # Call SentimentalAnaliser and accept multiple possible return shapes
            res = f2.get_user_activity_sentiment(full, num_events=num_events, token=token)

            if isinstance(res, (tuple, list)):
                # expected shape: (avg, scores_list)
                s_avg = float(res[0]) if res and res[0] is not None else 0.0
                s_list = res[1] if len(res) > 1 and isinstance(res[1], (list, tuple)) else []
            else:
                # older behavior: a plain float
                s_avg = float(res)
                s_list = []

            sentiments[full] = {'score': s_avg, 'n': len(s_list)}
        except RuntimeError as e:
            # specific runtime errors (for example authentication problems) are recorded
            logger.error(f"Runtime error while fetching sentiment for {full}: {e}")
            sentiments[full] = {'score': 0.0, 'n': 0, 'error': str(e)}
        except Exception as e:
            logger.warning(f"Failed to compute sentiment for {full}: {e}")
            sentiments[full] = {'score': 0.0, 'n': 0}

    # average available sentiment scores (exclude repos with score == 0.0 if they were empty)
    valid_scores = [v['score'] for v in sentiments.values() if v.get('score') is not None and v.get('score') != 0.0]
    if valid_scores:
        avg = statistics.mean(valid_scores)
    else:
        # if all zero or none, fallback to mean of all numeric scores (keeps previous behaviour)
        all_scores = [v['score'] for v in sentiments.values() if isinstance(v.get('score'), (int, float))]
...
def score_user(username: Optional[str] = None, token: Optional[str] = None,
               ...
               num_events_sentiment: int = 10) -> Dict[str, Any]:
    """Main entry point.
